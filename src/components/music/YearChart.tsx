      ...
      <div className="flex gap-1 sm:gap-2 h-48 sm:h-56">
        {byYear.map(({ year, hours }) => {
          ...
          return (
            <div
              key={year}
              className="flex-1 flex flex-col items-center min-w-0 h-full group"
            >
              <span className="text-[10px] sm:text-xs text-gray-500 dark:text-spethial-muted mb-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                {hours}h
              </span>
              ...
              <span className="text-[10px] sm:text-xs text-gray-600 dark:text-spethial-muted mt-2 truncate w-full text-center shrink-0">
                {String(year).slice(2)}
              </span>
            </div>
          )
