...
export default function MarkdownEditor({ value, onChange }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex flex-wrap gap-1 p-2 bg-sp-bg-elev-1 border border-sp-border rounded-t-lg">
        {toolbarButtons.map((btn, i) => 
          'divider' in btn ? (
            <div key={i} className="w-px bg-sp-border mx-1" />
          ) : (
            <button
              key={btn.label}
              type="button"
              onClick={() => insertText(btn.before, btn.after)}
              className={`px-2 py-1 text-sm bg-sp-bg border border-sp-border rounded hover:bg-sp-bg-elev-1 transition-colors ${btn.className || ''}`}
              title={btn.title}
            >
              {btn.label}
            </button>
      ...
      <textarea
        ...
        className="flex-1 w-full p-4 font-mono text-sm bg-sp-bg border border-t-0 border-sp-border rounded-b-lg resize-none focus:outline-none focus:ring-2 focus:ring-sp-teal text-sp-ink placeholder-gray-400"
